...
class IntegrationPluginSpec extends AbstractIntegrationSpec {
    ...
    public static final String buildFileContentMultiRepo = """
            plugins {
                id 'com.intershop.gradle.cisetup'
            }
            ...
            IntershopCISetup {
                ...
                repository {
                     ...
                     */
                    ivyRepoReleasesPath = 'ivy-releases-group'
                     ...
                     * Default: ivy-releases
                     */
                    mvnRepoReleasesPath = 'mvn-releases-group'
                     ...
                     * Default: ivy-releases
                     */
                    ivyRepoSnapshotsPath = 'ivy-snapshots-local'
                     ...
                     * Default: ivy-releases
                     */
                    mvnRepoSnapshotsPath = 'mvn-snapshots-local'
    ...
    def "Run corporate distribution task with multirepo configuration"() {
        given:
        String[] taskList = [':createCorporateDistribution']
        ...
        File repoRoot = new File(testProjectDir, 'testRepo')
        buildFile << buildFileContentMultiRepo.replace('http://nexus:8081/nexustest/repositories', "${repoRoot.toURI().toURL()}/nexustest")
        ...
        File initGradle = new File(testProjectDir, 'intershop-ci-setup/devops/gradle/corporate-distribution/src/init.d/intershop-init.gradle')
        ...
        initGradle.exists()
        initGradle.text.contains("${repoRoot.toURI().toURL()}/nexustest/ivy-releases-group")
        initGradle.text.contains("${repoRoot.toURI().toURL()}/nexustest/mvn-releases-group")
        initGradle.text.contains("${repoRoot.toURI().toURL()}/nexustest/mvn-snapshots-local")
        initGradle.text.contains("${repoRoot.toURI().toURL()}/nexustest/mvn-snapshots-local")
